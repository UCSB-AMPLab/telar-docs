...
| `clip_start` | `inicio_clip` | No | Inicio del clip en segundos (ej., `12.5`) |
| `clip_end` | `fin_clip` | No | Fin del clip en segundos |
| `loop` | `bucle` | No | Reproducción en bucle (`true`, `false`, `yes`, `no`, `sí`). Predeterminado: false |
| `alt_text` | `texto_alt` | No | Texto alternativo por paso para accesibilidad |
...
#### loop / bucle
- **Nuevo en v1.0.0**
- Controla si la reproducción de video o audio se repite en bucle
- Acepta `true`, `false`, `yes`, `no`, `sí`
- Predeterminado: `false` si se deja vacío
- Se ignora para objetos de imagen

#### alt_text / texto_alt
- **Nuevo en v1.0.0**
- Texto alternativo por paso para accesibilidad
- Si se deja vacío, usa el texto alternativo del objeto (de `objects.csv`) y luego el `title` del objeto
- Úsalo para proporcionar contexto específico de lo que muestra el paso — ej., un detalle ampliado que difiere de la descripción general del objeto
